...
export class AESAlgorithm extends CryptoAlgorithm {
  name = 'AES'
  displayName = 'AES'
  type = AlgorithmType.SYMMETRIC
  description = 'AES 高级加密标准，支持 128/192/256 位密钥'
  supportDecrypt = true
  ...
  protected validateOptions(options?: CryptoOptions, _operation?: 'encrypt' | 'decrypt'): string | null {
    ...
    const keyLen = options.key.length
    if (keyLen !== 16 && keyLen !== 24 && keyLen !== 32) {
      return '密钥长度必须为 16、24 或 32 字节（对应 128/192/256 位）'
    }
    ...
    const commonOptions = [
      {
        ...
        description: '密钥长度决定加密强度：16字节=128位，24字节=192位，32字节=256位'
      },
      {
        key: 'iv' as const,
        label: 'IV（初始向量）',
        type: 'text' as const,
        placeholder: '16 字节 IV（ECB 模式不需要）',
