...
export class SHA384Algorithm extends CryptoAlgorithm {
  name = 'SHA384'
  displayName = 'SHA-384'
  type = AlgorithmType.HASH
  description = 'SHA-384 安全哈希算法，生成384位哈希值'
  ...
  protected async encryptCore(input: string, options?: CryptoOptions): Promise<string> {
    ...
    if (window.crypto && window.crypto.subtle) {
      try {
        ...
        const hashBuffer = await window.crypto.subtle.digest('SHA-384', buffer)
